...
pub fn load(allocator: std.mem.Allocator) !*const Config {
    ...
    config.* = Config{
        ...
        // Server defaults
        .host = getEnvOrDefault("FORMDB_HOST", "127.0.0.1"),
        .port = getEnvPort("FORMDB_PORT", 8080),
        .version = "0.0.4",

        // Auth defaults
        .jwt_secret = std.posix.getenv("FORMDB_JWT_SECRET"),
        .api_key_header = getEnvOrDefault("FORMDB_API_KEY_HEADER", "X-API-Key"),
        ...
        // TLS
        .tls_cert_path = std.posix.getenv("FORMDB_TLS_CERT"),
        .tls_key_path = std.posix.getenv("FORMDB_TLS_KEY"),
    };
