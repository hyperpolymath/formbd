...
*Dependencies*: Phase 8 (Production Hardening) complete.

=== Deliverables

* [ ] API server binary (`formdb-server`) in Zig
* [ ] REST API with OpenAPI 3.1 specification
* [ ] gRPC API with Protocol Buffers definitions
* [ ] GraphQL API with SDL schema
* [ ] Authentication middleware (JWT, API keys, mTLS)
...
type Query {
  ...
  query(fdql: String!, provenance: ProvenanceInput): QueryResult!
}

type Mutation {
  createCollection(name: String!, schema: SchemaInput!): Collection!
  execute(fdql: String!, provenance: ProvenanceInput!): MutationResult!
...
* All FDQL operations accessible via REST, gRPC, and GraphQL
* Provenance tracking across all protocols
* Streaming support for journal and large result sets
