...
/**
 * FormDB Payload CMS Hooks
 *
 * Collection hooks for syncing Payload content to FormDB
 */
...
let makeClient = (config: pluginConfig): formdbClient => {
  ...
  let request = async (fdql: string): Js.Json.t => {
    let url = config.formdbUrl ++ "/v1/query"
...
/** Get FormDB collection for Payload slug */
let getFormDBCollection = (payloadSlug: string): option<string> => {
...
/** Check if collection should sync to FormDB */
let shouldSyncToFormDB = (payloadSlug: string): bool => {
  switch Js.Dict.get(state.mappings, payloadSlug) {
  | Some(mapping) =>
    switch mapping.syncMode {
    | Bidirectional | PayloadToFormDB => true
    | FormDBToPayload => false
    }
  | None => false
  }
}
...
let afterChangeHook = async (args: hookArgs<Js.Json.t>): hookResult<Js.Json.t> => {
  ...
  if shouldSyncToFormDB(slug) {
    switch (state.client, getFormDBCollection(slug)) {
