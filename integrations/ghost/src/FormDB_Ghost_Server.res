...
/**
 * FormDB Ghost Webhook Server
 *
 * Deno HTTP server for receiving Ghost webhooks
 */

open FormDB_Ghost_Types
open FormDB_Ghost_Webhook
...
let handleRequest = async (config: integrationConfig, request: {..}): {..} => {
  ...
  if method === "GET" && String.includes(url, "/health") {
    return createResponse(200, `{"status":"ok","service":"formdb-ghost"}`)
  }
...
let main = (): unit => {
  ...
  let config: integrationConfig = {
    formdbUrl: %raw(`Deno.env.get("FORMDB_URL") || "http://localhost:8080"`),
    apiKey: %raw(`Deno.env.get("FORMDB_API_KEY") || undefined`),
