...
/**
 * FormDB Directus Hook Extension
 *
 * Directus hook for syncing content to FormDB
 */

open FormDB_Directus_Types

/** Node.js fetch binding */
@val external fetch: (string, {..}) => promise<{..}> = "fetch"

/** Environment variable access */
@val @scope("process.env") external formdbUrl: option<string> = "FORMDB_URL"
@val @scope("process.env") external formdbApiKey: option<string> = "FORMDB_API_KEY"
@val @scope("process.env") external formdbSyncCollections: option<string> = "FORMDB_SYNC_COLLECTIONS"

/** Create FormDB client */
let makeClient = (~baseUrl: string, ~apiKey: option<string>=?): formdbClient => {
  let headers = {
...
let createHook = (): hookDefinition => {
  let baseUrl = formdbUrl->Option.getOr("http://localhost:8080")
  let client = makeClient(~baseUrl, ~apiKey=formdbApiKey)
