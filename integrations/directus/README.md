# FormDB Directus Extension

Directus extension for synchronizing content with FormDB.
...
npm install @formdb/directus-extension

# Or copy to extensions folder
cp -r dist/* extensions/hooks/formdb/
```
...
FORMDB_SYNC_COLLECTIONS=articles,authors,categories  # Comma-separated, empty = sync all
```
...
| Event | Action |
|-------|--------|
| `items.create` | Insert into FormDB |
| `items.update` | Update in FormDB |
| `items.delete` | Delete from FormDB |
...
FORMDB_SYNC_COLLECTIONS=articles,products,orders
```
...
| `FORMDB_SYNC_COLLECTIONS` | Collections to sync (comma-separated) | All non-system |

## Architecture

```
src/
├── FormDB_Directus_Types.res  # Type definitions
└── FormDB_Directus_Hook.res   # Hook extension
```
...
1. Check collection is in `FORMDB_SYNC_COLLECTIONS` (or variable is unset)
...
- Setting up FormDB connection pooling
