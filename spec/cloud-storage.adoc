...
[.lead]
FormDB is designed from the ground up to be safe on cloud storage services. This specification documents the guarantees, conflict resolution strategies, and recovery mechanisms.

toc::[]

== Overview

FormDB's append-only journal architecture makes it inherently safe for cloud storage, unlike traditional databases that use random-write patterns.
...
[source,text]
----
FormDB journal on cloud storage:
...
FormDB is designed for extended offline use:
...
FormDB does NOT rely on file locking:
...
FormDB approach (WORKS on cloud):
...
[cols="1,1,1,1,1"]
|===
| Feature | FormDB | SQLite | CouchDB | Git
...
| Q-CLOUD-001
| Should FormDB have built-in encryption?
...
* https://github.com/hyperpolymath/zotero-formdb[Zotero-FormDB] - Reference manager using cloud storage
